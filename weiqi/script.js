/**
 * 1132916 - FinalTerm 最終繳交版 (提子動畫強化版)
 */
...
let capturedSet = new Set(); // 暫存提子動畫中的座標
...
function handleMove(r, c) {
    ...
    if (result.success) {
        ...
        if (result.capturedCoords.length > 0) {
            ...
            renderBoard(); // 渲染帶有 .captured 類別的棋子
...
function handlePass() { passCount++; showMessage(`${currentPlayer === BLACK ? '黑棋' : '白棋'} Pass`); if (passCount >= 2) endGame(); else { currentPlayer = (currentPlayer === BLACK) ? WHITE : BLACK; updateStatus(); const mode = document.getElementById('game-mode').value; if (mode === 'PvC') checkAiTurn(); } }
